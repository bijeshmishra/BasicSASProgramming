	...
	*** TEMPHUMIDWIND21.SAS *** ;
	...
	TITLE "Bijesh Mishra, Assignment 9, Problem 1 "; 
	data meso ; 
...
TITLE "Bijesh Mishra, Assignment 9, Problem 1(a) ";
LIBNAME ClassHw " C:\Users\bmishra\Dropbox\OSU\PhD\Fall 2021\STAT5193 SAS R\SAS\Assignments\Assignment 9";
DATA CLASSHW.MESONET2021; 
SET meso;
RUN; QUIT;

TITLE "Bijesh Mishra, Assignment 9, Problem 1(b) ";
PROC SORT DATA = ClassHw.MesoNet2021;
BY month;
PROC REPORT DATA = ClassHw.MesoNet2021;
COLUMN month maxtemp maxhumid;
BY month;
DEFINE maxtemp / "Maximum Temperature" MEAN;
DEFINE maxhumid / "Maximum Humidity" MEAN;
DEFINE Month / GROUP;
RUN; QUIT;

TITLE "Bijesh Mishra, Assignment 9, Problem 2(a) ";
DATA CLASSHW.ARCH;
...
LABEL SUBJ = " Individuals "
      ...
      FSH = " Fashion ";
RUN; QUIT;
* PROC PRINT DATA = CLASSHW.ARCH (OBS = 10) NOOBS LABEL;
* RUN; * QUIT;

TITLE "Bijesh Mishra, Assignment 9, Problem 2(b) ";
PROC GCHART DATA = ClassHw.Arch;
PIE int / TYPE = PERCENT;
RUN; QUIT;

TITLE "Bijesh Mishra, Assignment 9, Problem 3 ";
DATA CIINT;
Mean = 94.22;
StdErr = 6.90;
ssize = 18;
t = TINV(0.97, 17);
t1 = TINV((1 - 0.025), 1000); /* = 1.96; Gives Probability */
cilower = mean - t*StdErr;
ciupper = mean + t*StdErr;
f = 1- PROBF (4.47, 4, 14); /* Gives P-value Or Significance Level */
LABEL Mean    = "Mean"
      cilower = "CI Lower Limit"
	  stderr = "Standard Error"
	  ssize = "Sample Size"
	  ciupper = "CI Upper Limit"
	  t = "T-Critical"
	  t1 = "TINV (0.025, 1000)"
	  f = "F-Dist. P-value (Q3b)";
;
PROC PRINT DATA = CIINT LABEL NOOBS;
VAR Mean StdErr ssize t cilower ciupper f;
RUN; QUIT;
